...
% =========================================================================
nx = 1+7+10+1; % 原来：1+6+9+1; % # of states: time, 6 states, 9 auxilliary states, objective
nu = 3; % 原来：2; % # of inputs
...
u_LB = [zeros(1,N+1); [zeros(1,N-11) Params.optlrsav*ones(1,11) 0]; zeros(1, N+1)];
u_UB = [[1 1.0*ones(1,27) Params.limmiu*ones(1,N-27)]; ones(1,N+1); ones(1, N+1)];
...
x_UB(end-3:end-1, :) = 1;  % upper bound on shifted states (inputs)
...
x_LB_ini(end-3:end-1, N+2) = [0; 0; 0];
x_UB_ini = 1E12*ones(nx, N+2); % no upper bound on states
x_UB_ini(2,:) = Params.T_AT_max; % temperature constraint

% To reproduce Nordhaus DICE2013 results, uncomment lines below
x_UB_ini(end-3:end-1, :) = Params.limmiu;  % upper bound on shifted states, i.e. inputs
x_UB_ini(end-3:end-1, N+2) = [Params.miu0; 1; 1];  % initial input constraints in
